...
export default function NvxLiens({ refresh, setLienIsVisible }) {
  const [liens, setLiens] = useState([{href:'', description:'', motsClef:[''], salle:'' }]);
  const [alert, setAlert] = useState(false);
  const [loading, setLoading] = useState(false);

  const newLiens = async () => {
    setLoading(true);
    if (!name) {
      setLoading(false);
      setAlert(true);
